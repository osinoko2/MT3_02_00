...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		Matrix4x4 startWorldMatrix = MakeAffineMatrix({ 1.0f, 1.0f, 1.0f }, { 0.0f, 0.0f, 0.0f }, segment.origin);
		Matrix4x4 endWorldMatrix = MakeAffineMatrix({ 1.0f, 1.0f, 1.0f }, { 0.0f, 0.0f, 0.0f }, Add(segment.origin, segment.diff));
		...
		Vector3 startNdcMatrix = Transform({ 0.0f, 0.0f, 0.0f }, startWorldviewProjectionMatrix);
		Vector3 endNdcMatrix = Transform({ 0.0f, 0.0f, 0.0f }, endWorldviewProjectionMatrix);

		Vector3 startScreenMatrix = Transform(startNdcMatrix, viewportMatrix);
		Vector3 endScreenMatrix = Transform(endNdcMatrix, viewportMatrix);
		...
		Novice::DrawLine(int(startScreenMatrix.x), int(startScreenMatrix.y), int(endScreenMatrix.x), int(endScreenMatrix.y), WHITE);
		DrawSphere(pointShere, viewProjectionMatrix, viewportMatrix, RED);
...
{
	Vector3 a;
	Vector3 b = Normalize(segment.diff);
	float t;
	t = (point.x - segment.origin.x) * b.x + (point.y - segment.origin.y) * b.y + (point.z - segment.origin.z) * b.z;
	a = { t * b.x + segment.origin.x, t * b.y + segment.origin.y, t * b.z + segment.origin.z };
